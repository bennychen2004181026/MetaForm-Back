...
const getAllResponsesByFormId: RequestHandler = async (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
    const { id } = req.params;
    if (!id) {
        return res.status(404).json({ message: 'Provide form id' });
    }
    const formResponses = await FormResponse.find({ formId: id }).exec();
